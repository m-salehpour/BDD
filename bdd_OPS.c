...
#define bit_predicate_s   15 //14//32   //14 //9
#define bit_object_s      21 //20//64   //20 //15


#define bit_subject_e     14 //31//13 //8  
#define bit_predicate_e   20 //63//19 //14
#define bit_object_e      34 //127 //24

bdd kg;
static const int varnum = 35; //128; // 35; //25; //limited BSBM
/*
 *
...
void ___bdd_construct(int _ss_b, int _pp_b, int _oo_b)
{
	...
	//binS are switched to accommodate OPS

	char * bin_s = dec2bin(bit_subject_e, _oo_b ); 
	char * bin_p = dec2bin( ( (bit_predicate_e) - (bit_predicate_s)  ), _pp_b );
	char * bin_o = dec2bin( ( (bit_object_e) - (bit_object_s)  ), _ss_b );
	...
	//bin_SO[0] = '2';
		...
		//bin_SO[SO_counter++] = bin_p[i-(bit_subject_e+1)];
...
void allsatSPO()
{
	...
	for (long k=1; k < pow(2, (bit_object_s) ); k++ )
	{
		bin_q = dec2bin(bit_predicate_e, k );
...
int main(void)
{
	...
	bdd_init(10000000000,100000000); // 40G
	//
