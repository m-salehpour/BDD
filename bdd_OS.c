...
#define bit_predicate_s   16 //14//32   //14 //9
#define bit_object_s      29 //20//64   //20 //15


#define bit_subject_e     15 //31//13 //8  
#define bit_predicate_e   30 //63//19 //14
#define bit_object_e      34 //127 //24

bdd kg;
static const int varnum = 31; //128; // 35; //25; //limited BSBM
/*
 *
...
void allsatSPO()
{
	...
	*/
        ...
        for (long k=1; k < bin_SP_counter  ; k++ )
        {
                ...
                for (int v=0; v < bit_predicate_s ; v++)
                {
                        if (bin_q[v] == '0')
                                alpha = bdd_addref(bdd_apply( bdd_nithvar(v) , alpha , bddop_and ));    
                        else
